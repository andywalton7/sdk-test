...
* **[Using delivery options to create a pack order flow](#creating-a-pack-order-flow-from-delivery-options)**
   ...
   Used when you can't guarantee that all parts of a customer's order will come from the same warehouse and you want to present delivery options to your customer at point of purchase.

* **[Obtaining and selecting delivery quotes](#selecting-quotes)**

   Generally used in customer service use cases, where an operator needs to get delivery options for a customer manually and act on the customer's response.
...
Finished! The next section explains a similar process, whereby the order is created up front rather than generated from delivery options that the customer selects.
